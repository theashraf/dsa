...
class Trie {
  ...
  constructor() {
    this.children = Array<Trie>(26) // to hold 26 characters a-z;
  ...
  insert(str: string): void {
    ...
    const current: number = str.slice(0, 1).charCodeAt(0) - 'a'.charCodeAt(0)
  ...
  searchWord(word: string): boolean {
    ...
    const current: number = word.slice(0, 1).charCodeAt(0) - 'a'.charCodeAt(0)
  ...
  searchPrefix(prefix: string): boolean {
    ...
    const current: number = prefix.slice(0, 1).charCodeAt(0) - 'a'.charCodeAt(0)
